      ...
      <section className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex justify-center items-center flex-col">
        ...
        <div className="relative w-[280px] h-[580px] border-[8px] border-slate-900 rounded-[36px] bg-slate-50 overflow-hidden shadow-xl ring-1 ring-slate-200 flex flex-col">
          ...
          <div className="bg-white px-4 pt-8 pb-3 border-b border-slate-100 flex items-center justify-between sticky top-0 z-0 shadow-sm">
             <span className="material-symbols-outlined text-slate-800 text-xl font-bold">menu</span>
             <h4 className="text-sm font-bold text-slate-800">{pageName || "App Page"}</h4>
             <span className="material-symbols-outlined text-slate-800 text-xl">search</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6 relative hide-scrollbar">
             ...
             <div className="space-y-3">
               <div className="flex items-center justify-between">
                 <h5 className="font-bold text-sm text-slate-800">{sectionName || "Section"}</h5>
                 <span className="text-[10px] text-[#1325ec] font-semibold">View All</span>
               </div>
               
               {/* Display based on format */}
               {!collectionId ? (
                 <div className="h-32 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center bg-white">
                    <p className="text-xs font-medium text-slate-400">Select collection mapping</p>
                 </div>
               ) : isBanner ? (
                 ...
                 <div className="w-full aspect-[2/1] rounded-xl overflow-hidden bg-slate-200 shadow-sm">
                    {imageUrl ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-slate-400 text-xs">Banner Image</div>
                    )}
                 </div>
               ) : (
                    ...
                    <div className="w-32 flex-shrink-0 space-y-2">
                       <div className="w-full aspect-[3/4] rounded-xl overflow-hidden bg-slate-200 shadow-sm">
                         {imageUrl ? (
                           ...
                           <div className="w-full h-full flex items-center justify-center text-slate-400 text-[10px]">Portrait Image</div>
                         )}
                       </div>
                       <p className="text-xs font-bold text-slate-800">Collection Item</p>
                    </div>
                    {/* Dummy extra item */}
