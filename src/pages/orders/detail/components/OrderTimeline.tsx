...
export default function OrderTimeline({ order }: OrderTimelineProps) {
  const logs = [...(order.statusLogs || [])].sort((a, b) => 
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm h-fit">
      ...
      <div className="space-y-6">
        {logs.length === 0 ? (
           <p className="text-sm text-slate-500 italic text-center py-4">No status updates yet.</p>
        ) : (
          logs.map((log, index) => (
            <div key={index} className="flex gap-4">
              <div className="flex flex-col items-center">
                <div className={`size-2.5 rounded-full mt-1.5 ${index === 0 ? 'bg-[#1325ec]' : 'bg-slate-300'}`} />
                {index !== logs.length - 1 && (
